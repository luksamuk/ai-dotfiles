    ...
    def __call__(self, components: MiniMaxMusic3ModularPipeline, state: PipelineState) -> PipelineState:
        ...
        for frame_index in range(max_frames + 1):
            if frame_index % 10 == 0 and frame_index > 0:
                _now = time.time()
                _elapsed = _now - _progress_start
                _frame_rate = frame_index / _elapsed if _elapsed > 0 else 0
                _eta = (max_frames - frame_index) / _frame_rate if _frame_rate > 0 else 0
                _audio_secs = frame_index / components.frame_rate
                print(f"  [Music3] frame {frame_index}/{max_frames} ({_audio_secs:.1f}s audio) "
                      f"| {_frame_rate:.2f} fps | elapsed {_elapsed:.0f}s | ETA {_eta:.0f}s", flush=True)
                _progress_last = _now
